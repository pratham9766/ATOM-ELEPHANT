...
export function CheckinWorkspace() {
  ...
  async function save(goal: ApiGoal) {
    await saveCheckin.mutateAsync({
      ...
      actual_value: actuals[goal.id] ?? 0,
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-slate-400">Window status: open for demo cycle</div>
            </div>
          </div>
            ...
            {goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="grid grid-cols-[1.2fr_.7fr_.8fr_.8fr_1fr_.6fr] items-center gap-4 py-4">
                  ...
                  <div className="text-sm text-slate-300">{goal.target ?? "0"}</div>
                  <input
                    type="number"
                    value={actual}
                  ...
                  <div>
                    <div className="mb-1 text-xs text-slate-400">
                      {actual} / {goal.target ?? 100} = {score}%
                    </div>
                    <Progress value={Math.min(score, 100)} />
                  </div>
                  <Button size="sm" disabled={saveCheckin.isPending || sheet?.status !== "locked"} onClick={() => save(goal)}>
                    <Save size={14} />
                    Save
                  </Button>
                </div>
